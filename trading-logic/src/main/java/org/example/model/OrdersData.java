...
import static org.example.service.UniversalKlineCandleProcessorImpl.ROUND_SIGN_QUANTITY;
...
public record OrdersData(Map<FibaLevel, BigDecimal> fibaLevelsToCompare,
    ...
    public Order amendOrderPriceTpSl(Map<FibaLevel, BigDecimal> levelPrice) {
        order.setPrice(getPrice(levelPrice.get(FIVE), ROUND_SIGN_QUANTITY));
        order.setStopLoss(getPrice(levelPrice.get(ONE), ROUND_SIGN_QUANTITY));
        order.setTakeProfit(getPrice(levelPrice.get(THREEEIGHTTWO), ROUND_SIGN_QUANTITY));
